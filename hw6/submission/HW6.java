...
public class HW6 {
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaRDD<Row> fromSeattle = d.filter(t -> t.get(ORIGIN_CITY_NAME).toString().equals("Seattle, WA"));
    JavaRDD<Row> destCity = fromSeattle.map(t -> RowFactory.create(t.get(DEST_CITY_NAME)));
    JavaRDD<Row> result = destCity.distinct();
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaRDD<Row> filtered = d.filter(t -> t.get(CANCELLED).equals(0));
    JavaPairRDD<Tuple2<String, Integer>, Integer> temp =
    filtered.mapToPair(t -> new Tuple2<Tuple2<String, Integer>, Integer>
    (new Tuple2<String, Integer>(t.get(ORIGIN_CITY_NAME).toString(), (int) t.get(MONTH)), 1));
    JavaPairRDD<Tuple2<String, Integer>, Integer> result = temp.reduceByKey((v1, v2) -> v1 + v2);
    // result.foreach(t -> System.out.println(t));
    return result;
  }
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaPairRDD<String, String> filtered = d.mapToPair(t ->
     new Tuple2<String, String>(t.get(ORIGIN_CITY_NAME).toString(), t.get(DEST_CITY_NAME).toString())).distinct();
    JavaPairRDD<String, Integer> oricity = filtered.mapToPair(tuple -> new Tuple2<String, Integer>(tuple._1(), 1));
    JavaPairRDD<String, Integer> count = oricity.reduceByKey((v1, v2) -> v1 + v2);
    String max_city = count.max(new TupleComparator())._1();
    JavaRDD<Row> target = d.filter(t -> t.get(ORIGIN_CITY_NAME).toString().equals(max_city));
    return new Tuple2<String, Integer>(max_city, (int) (long) target.count());
  }
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaRDD<Row> filtered = d.filter(t -> t.get(DEP_DELAY) != null);
    JavaPairRDD<String, Tuple2<Integer, Double>> temp = filtered.mapToPair(t ->
    new Tuple2<String, Tuple2<Integer, Double>>(t.get(ORIGIN_CITY_NAME).toString(),
                                              new Tuple2<Integer, Double>((int) t.get(DEP_DELAY), 1.0)));
    temp = temp.reduceByKey((v1, v2) -> new Tuple2<Integer, Double>(v1._1() + v2._1(), v1._2() + v2._2()));
    JavaPairRDD<String, Double> result = temp.mapToPair(t ->
    new Tuple2<String, Double>(t._1(), t._2()._1() / t._2()._2()));
    // result.foreach(t -> System.out.println(t));
    return result;
  }
